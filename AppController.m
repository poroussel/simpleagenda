...
{
  return [[(Event *)[a valueForKey:@"object"] startDate] compare:[(Event *)[b valueForKey:@"object"] startDate] withTime:YES];
}
...
- (void)updateSummaryData
{
  ...
  [_tasks setValue:[NSString stringWithFormat:_(@"Open tasks (%llu)"), [[_tasks children] count]] forKey:@"title"];
  [summary reloadData];
}
...
NSComparisonResult compareEventTime(Event  *a, Event *b, void *context)
{
...
- (void)performSearch
{
  ...
  if ([[search stringValue] length] > 0) {
    ...
    [_results setValue:[NSString stringWithFormat:_(@"Search results (%llu items)"), [[_results children] count]] forKey:@"title"];
  } else
